...
public class Transform {
	...
	public Term createAnd(List<Term> l) {
		...
		for (int i=0; i<l.size(); i++) {
			...
			if (!tip.equals(new BoolC(true))) lp.add(tip);
		}

		if (lp.size() == 0) t = new BoolC(true);
		else if (lp.size() == 1) t = lp.get(0);
		else t = new And(lp);
		...
		if (t.isBin()) {
			...
			if ((tb.getOp().equals("=") || tb.getOp().equals("!=")) && 
				 ...
				 if (isUnionVar(tb.getT1())) {
					...
					/*else  // var = cte
						ct = eqVarCt((Var)tb.getT1(), (CTerm)tb.getT2());*/
				 } 
						 
				if (tb.getOp().equals("=")) cp = new Constraint(ct);
				else cp = new Constraint(new UnaryExp("not",ct));
		...
		if (v1.getT().rhsString().equals(v2.getT().rhsString())) {
			try {
		      ...
		      for (Var aux1:l1) {
		    	  ...
		    	  String aux1s2 = aux1s.replaceFirst(v1.getName(), v2.getName());
				...
				System.out.println("Unexpected error comparing union variables "+v1.getName()+" of type "+v1.getT().rhsString()
					           + " and "+v2.getName()+" of type "+v2.getT().rhsString());
			}

		} else
			System.out.println("Error comparing union variables "+v1.getName()+" of type "+v1.getT().rhsString()
					           + " and "+v2.getName()+" of type "+v2.getT().rhsString());
		return t;
	}
	...
	public Term transShow(Term t) throws Exception {
		...
		if (t.isShow())
			 ...
			 st = t;
				...
				/*
				CTerm ct = (CTerm)t;
				Term Left =  new StringC(ct.getCons());
				Term aux = null;
				for (Term Arg:ct.getArgs()) {
					Term trec = transShow(Arg);
					aux = Left;
					Left = new BinExp("++", aux, trec);
				}
				st = Left;
				*/
				st = t;
		...
		Term t = null;
			...
			if (d==null)
			   throw new Exception("(sVar) Unexpected union type name "+typename);
				...
				ls.add(new StringC(v.getName()) );
				ls.add(new StringC(": ") );
				ls.add(sVari(d,v,l,0,n));
				ls.add(new StringC("\n ") );
			    t = new BinExp("++",ls);*/
			}
