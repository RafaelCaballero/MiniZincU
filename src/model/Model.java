...
/*
	...
	public Var getVarByName(String name) {
		...
		for (Var v2:var) {
			if (v2.getName().equals(name)) v = v2;
		}
...
*/
	...
	public  String toString(){
		...
		// output
		s+="output([";
		int noutput = output.size();
		for(int i=0; i<noutput; i++){ 
		  Term sh = output.get(i);  
		  s += sh; 
		  if (i<noutput-1) s+= ", ";
		}
		s+="]);";
	...
	private void showVars() {
		for(Var itv:var) {
			List<Term> lt =  Arrays.asList(new Term[]{new StringC("\\n "),
					                       new StringC(itv.getName()), 
					                       new StringC(": "),
					                       new Show(itv)});
			
			BinExp itvs = new BinExp("++", lt, lt.size() );
	...
	public void setCtr(List<Constraint> ctrs) {
		this.ctrs = ctrs;
	}

	public void addCtr(@NotNull Constraint ctr) {
		this.ctrs.add(ctr);
	}
	...
	public void setOutput(List<Term> output) {
		this.output = output;
	}
