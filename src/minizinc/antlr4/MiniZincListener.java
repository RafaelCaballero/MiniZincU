...
public class MiniZincListener extends MiniZincGrammarBaseListener {
	...
	/**
	 * @param parser
	 *            : The ANTLR4 grammar parser
	 * @param pu
	 *            : An empty model that will be completed after the parsing
	 */
	...
	/**
	 * After a data statement has been parsed the program will contain a new
	 * SDataDef statement
	 * 
	 * @see minizinc.antlr4.MiniZincGrammarBaseListener#exitData(minizinc.antlr4.MiniZincGrammarParser.DataContext)
	 */
	@Override
	public void exitData(@NotNull MiniZincGrammarParser.DataContext ctx) {
		...
		int n = lconstr.size();
		if (n == 0)
			System.out.println("ERROR: data type " + id
			...
			for (ArgContext arg : ltypes) {
				...
				if (af != null)
					...
					try {
						...
						System.out
								.println("ERROR: unexpected variable range type "
										+ c.toString());
	...
	public void exitVar(@NotNull MiniZincGrammarParser.VarContext ctx) {
		...
		if (tint != null)
			...
			System.out.println("ERROR: Union type variable without level "
					+ ctx.toString());
		else if (tdata != null) {
			...
			try {
				...
				System.out
						.println("ERROR: unexpected level in var. declaration "
								+ ctx.toString());
				// System.out.println(e);
			}
			...
			try {
				...
				System.out.println("ERROR: unexpected variable range type "
						+ ctx.toString());
				// System.out.println(e);
			}

		} else
			System.out.println("ERROR: unexpected variable type "
					+ ctx.toString());
		...
		case 1: // notExpr, minusExpr, INT, BOOLEXpr, predOrUnionExpr, idexpr
			if (e.notExpr() != null) {
				...
				try {
					...
					System.out.println("Error unexpected integer: "
							+ e.getText());
				}
			} else if (e.boolExpr() != null) {
				BoolExprContext cp = e.boolExpr();
				t = cp.getText().toUpperCase().equals("TRUE") ? new BoolC(true)
						: new BoolC(false);
				...
				String s = ((StringExprContext) e.stringExpr()).string()
						.getText();
				t = new StringC(s);
				...
				System.out.println("Error: unexpected " + n
						+ " params expression *" + e.getText() + "*");
		...
		if (d != null) {
			...
			if (n == m) {
				...
				System.out.println("Error: invalid number of arguments "
						+ cp.getText());
				System.out.println("       expected number of arguments: " + m
						+ ", actual number: " + n);
			}
		} else { // predicate
		...
		if (n == 1) { // either a constant or a boolean variable
			if (e.boolExpr() != null)
				if (e.getText().equals("true"))
					c = new Constraint(new BoolC(true));
				else
					c = new Constraint(new BoolC(false));
			else if (e.idexpr() != null) {
				String varname = e.getText();
				Var v = pu.getVarByName(varname);
				if (v.getT().getType() != TType.BOOL) {
					System.out
							.println("Error: invalid constraint type, non-bool identifier "
									+ varname);
				} else {
					;
				}
				...
				System.out.println("Error: invalid constraint type"
						+ e.getText());
			}
		} // n==1
		else
			/*
			 * if (n==3) { ParseTree t1 = e.getChild(1); if
			 * (t1.getText().equals("=")) { ExprContext e1 = e.expr(0);
			 * ExprContext e2 = e.expr(1); System.out.println(e1.getText());
	...
	public List<Term> getExpr(ListExprContext lc) {
		...
		if (lc.listValue() != null)
			...
			System.out.println("Error: unexpected list (" + lc.getText() + ")");
