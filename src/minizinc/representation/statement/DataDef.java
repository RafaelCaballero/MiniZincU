...
public class DataDef extends Statement {
	 ...
	 * @return A {@link DataConsData} value, or null if consname is not a
	...
	/**
	 * Adds a new subtype to a constuctor ad returns the position of the
	 * constraint *
	 */
	...
	/**
	 * A new sata statement representing an union type. Grammar:<br>
	 * data: 'enum' ID '=' '{' constr(','constr)* '}';
	 * 
	 * @param ctx
	 *            The context.
	...
	public static DataDef data(DataContext ctx) {
		...
		if (Parsing.hasTerminal(ctx.ID())) {
			...
			Parsing.error("Missing ID in data type definition! "
					+ ctx.getText());
