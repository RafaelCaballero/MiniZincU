 ...
 * In both cases the structure is similar: a type, an id and a possible
 * initialization.
 *
 * This abstract class contains the initialization expression, which can be
 * null. It is important to notice that equals only uses the identifier; we
 * assume that all the declarations use different names.
 * 
 * @author rafa
 *
 */
public abstract class Decl extends Statement {
	...
	protected static List<Type> getDimensions(DimensionsContext dctx) {
		...
		// if more than one int can be written as dimension the grammar should
		// change!!!
