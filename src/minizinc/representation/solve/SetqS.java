...
/**
 * Grammar: seqS : 'seq_search' '('? '[' modeAnnotation (','modeAnnotation)*
 * ']'')'?;
 *
 * @author rafa
 *
...
public class SetqS extends ModeAnnotation {
	 ...
	 * 
	 * @param l
	 *            List of ModeAnnotations in this object
	 */
	...
	public static SetqS setS(SeqSContext ctx) {
		...
		// ctx.modeAnnotation().stream().map(x ->
		// l.add(ModeAnnotation.modeannotation(x)));
		r = new SetqS(l);
		return r;
	}
