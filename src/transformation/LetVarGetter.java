...
public class LetVarGetter implements ExprTransformer {
	...
	public Expr transform(Expr input) {
		...
		if (input != null && input instanceof LetExpr) {
			...
			if (ll!=null) {
				Set<Decl> ld = ll.stream().filter(x->x.getStatement() instanceof Decl).map(x->(Decl) x.getStatement()).collect(Collectors.toSet());
				ids.addAll(ld.stream().map(x-> x.getID().print()).collect(Collectors.toSet()));
			}
			
		}
		
		// return the same input because this is not really a transformer but a "getter"
		return input;
	}
