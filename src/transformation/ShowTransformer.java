...
public class ShowTransformer implements ExprTransformer {
	...
	public Expr transform(Expr e) {
		...
		if (e instanceof PredOrUnionExpr) {
			...
			if (po.getId().print().toLowerCase().equals("show")) {
				// show must have only one argument, which must be a variable
				// name
				List<Expr> la = po.getArgs();
				if (la != null && la.size() == 1) {
					if (la.get(0) instanceof ID) {
						ID id = (ID) la.get(0);
						// get the variable declaration and check if it is a
						// data
						VarDecl v = m.getVarByName(id);
						if (v != null)
							r = transShow(v);
						else
							Parsing.error("Unexpected variable name: " + id
									+ " (" + e + ")");
					}
				}
			}
	 ...
	 * {@code if (fix(x)==i) then sThen else sElse}, one for each constructor
	 * {@code ci}. This method constructor the sThen sentence for the
	 * constructor ci
	 * 
	 * @param v
	 *            The variable we are transforming
	 * @param l
	 *            the level of the declaration
	 * @param i
	 *            The position of the constructor we are transforming (following
	 *            textual order in the data definition)
	 * @param ms
	 *            The arity of the constructor ci
	 * @param ci
	 *            The constructor for which we are building a then sentence
	 * @return An expression that corresponds to the then sentence of this
	 *         constructor
	 */
	...
	/**
	 * Adds the elements sVar (t i 1 , x i1 , l − 1, 1) to the list of
	 * expressions l
	 * 
	 * @param li
	 *            The list were the elements are added. Already initialized.
	 * @param v
	 *            The variable we are transforming
	 * @param l
	 *            The level of the declaration
	 * @param i
	 *            The index of the constructor in the datatype.
	 * @param ms
	 *            Number of arguments in the constructor
	 * @param c
	 *            The constructor
	 */
	private void addArgs(List<Expr> li, VarDecl v, int l, int i, int ms,
			DataCons c) {
